...
| Figures pass | **Done** | Added 10 new figures (`fig1_2`, `fig3_2`, `fig5_2`, `fig6_2`, `fig8_2`, `fig9_2`, `fig10_2`, `fig11_2`, `fig12_2`, `fig12_3`) covering the central refined concepts: phase invariance, cross-architecture universality, bimodal φ-cosine phase transition, HyperMapping 6-task benchmark, discriminant attention spectrum, 4-state holographic gate, two complementary spectra, Fibonacci correction decomposition, Platonic Ideal rotation, PCA cumulative variance. All embedded in their chapters with captions. Pre-planned `fig1_2_phase_invariance.py.todo` completed and renamed. (Final figure count after the zeta thread brought it to 26 — see "Zeta thread (Sessions 2–6)" below.) |
| Zeta thread (Sessions 2–6) | **Done** | Six-session integration of the σ = 1/2 critical-line theory as a self-contained Appendix B (B.0–B.9, ~330 lines), refined §5.3 in Chapter 5, four cross-chapter forward-references (Ch 8 §8.4, Ch 9 §9.5.1, Ch 10 §10.2.3, Ch 11 §11.4), and three new figures: `fig5_3_zeta_transformer` (Hardy Z(t) at 30 dps via mpmath alongside the Qwen2.5-7B 28-layer residual-stream projection), `figB_1_five_constraints` (Hardy 1914, RH bound, Borwein 7-term break, conditional-convergence partial sums, half-integer offset), `figB_2_empirical_zeros` (21 non-trivial zeros parsed automatically from `phi_collective_zero_hunt_results.txt`). Highest-fidelity rebuild in session 6: `fig5_3` panel A uses `mpmath.zeta(0.5+it)` at 30 dps with graceful Riemann–Siegel fallback; `figB_1` panel 4 numpy partial sums verified against mpmath at 50 dps to within 6×10⁻¹⁵ (no rebuild needed); `figB_2` parser regex extracts the Phase 10z summary table directly, with hand-transcribed fallback for hermeticity. Working notes preserved in `ZETA_THREAD_NOTES.md`. |
| Final build | **Done** | `bash scripts/build_paper.sh` runs cleanly. Output: **145-page PDF, 5.7 MB** (was 124 pp / 4.7 MB after the first refinement, 2.9 MB before any refinement). `paper.md` includes Appendix B as the closing chapter. Final figure inventory: **26 figures** across 12 chapters + Appendix B (was 13 originally, 23 after the first figures pass). Fixed LaTeX warnings during the integration: missing braces around `\tau_\max` operator subscript, inconsistent `\tfrac` in matplotlib mathtext (replaced with `\frac`), and a numerical inconsistency about the φ-Zipf exponent (0.481 is just below the [1/2, 1] band edge, not inside it). |
